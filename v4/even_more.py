...
squared_root = tf.keras.losses.MeanSquaredError()
...
def feature_matching(fake_fx, real_fx):
    return squared_root(fake_fx, real_fx)
...
@tf.function
def train_step(images, train_disc=True):
    noise = tf.random.normal([images.shape[0], noise_dim])
    # noise = seed
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_fe_tape, tf.GradientTape() as disc_final_tape:
        generated_images = generator(noise, training=True)

        real_fx = discriminator_fe(images, training=train_disc)
        fake_fx = discriminator_fe(generated_images, training=train_disc)

        real_output = discriminator_final(real_fx, training=train_disc)
        fake_output = discriminator_final(fake_fx, training=train_disc)
    ...
    if train_disc:
        ...
        discriminator_fe_optimizer.apply_gradients(
            zip(gradients_of_discriminator_fe, discriminator_fe.trainable_variables))
...
def train_gen(images):
    ...
    with tf.GradientTape() as gen_tape:
        ...
        real_fx = discriminator_fe(images, training=False)
        fake_fx = discriminator_fe(generated_images, training=False)

        fake_output = discriminator_final(fake_fx, training=False)
...
def train(epochs, starting_epoch):
