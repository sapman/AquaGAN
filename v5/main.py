...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv2D(8, (5, 5), strides=(1, 1), padding='same', input_shape=[140, 320, 3]))
    model.add(layers.LeakyReLU())
    ...
    model.add(layers.Conv2D(32, (5, 5), strides=(3, 3)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.5))
    ...
    model.add(layers.LeakyReLU(name='fx'))
...
def train_step(images):
    ...
    images_noise = tf.random.normal(images.shape, stddev=0.003)
    generated_noise = tf.random.normal(images.shape, stddev=0.003)
    # noise = seed
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        generated_images = generator(noise, training=True)

        real_output = discriminator(images + images_noise, training=True)
        fake_output = discriminator(generated_images + generated_noise, training=True)

        gen_loss = generator_loss(fake_output) # + feature_matching(generated_images, images)
